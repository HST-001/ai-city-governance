...
class ResNet18Adapter(StreetModel):
    """ResNet18模型适配器"""
    ...
    def _generate_recommendations(self, results):
        ...
        if results.get('store_sign', {}).get('level', 5) <= 2:
            recommendations.append('店招状况较差，建议进行整改。')
        
        if results.get('greenery', {}).get('level', 5) <= 2:
            recommendations.append('绿化覆盖率低，建议增加绿植。')
        
        if results.get('sidewalk', {}).get('level', 5) <= 2:
            recommendations.append('人行道铺装破损，建议及时修复。')
        
        if results.get('bike_lane', {}).get('level', 5) <= 2:
            recommendations.append('自行车道贯通性差，建议改善。')
        
        if results.get('urban_facilities', {}).get('level', 5) <= 2:
            recommendations.append('城市设施不足，建议增设。')
        
        if not recommendations:
            recommendations.append('街道环境整体状况良好，建议保持。')
    ...
    def get_model_info(self):
        ...
        return {
            ...
            'description': '基于ResNet18的街道环境评分模型'
        }
...
if __name__ == '__main__':
    # 测试ResNet18适配器
    adapter = ResNet18Adapter('models/model_test.pth')
