...
// 读取addressData.ts文件
const data = fs.readFileSync('./addressData.ts', 'utf8');

// 简单处理文件内容，移除前后的TypeScript语法
let jsonStr = data;
jsonStr = jsonStr.replace('const completeAddressData = ', '');
jsonStr = jsonStr.replace('export default completeAddressData;', '');
jsonStr = jsonStr.trim();

// 解析JSON
const addressData = JSON.parse(jsonStr);

// 检查指定地区的街道信息
function checkStreets(provinceLabel, districtLabel) {
  // 查找省份
  const province = addressData.find(p => p.label === provinceLabel);
  if (!province) {
