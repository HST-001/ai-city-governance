...
// 定义各省份的完整城市数据（包含所有地级市和省会城市）
const COMPLETE_PROVINCE_CITIES = {
  'beijing': {
    label: '北京市',
...
// 辅助函数：生成城市的value值
function generateCityValue(provinceValue, cityLabel) {
  // 将城市名称转换为拼音风格的value
  const cityValue = cityLabel.replace(/[市县区]/g, '').toLowerCase()
    .replace(/[\s\u4e00-\u9fa5]/g, ''); // 移除空格和汉字
  return `${provinceValue}-${cityValue}`;
}

// 辅助函数：清理重复的城市数据
function cleanDuplicateCities(province) {
  ...
  for (const city of province.children) {
    if (!seenLabels.has(city.label)) {
      seenLabels.add(city.label);
      uniqueCities.push(city);
    }
  }
  
  province.children = uniqueCities;
  return province;
}

// 辅助函数：补全省份的城市数据
function completeProvinceCities(province) {
  const provinceData = COMPLETE_PROVINCE_CITIES[province.value];
  if (!provinceData) {
    console.log(`未找到 ${province.label} 的完整城市数据`);
    return province;
  }
  ...
  // 记录已有的城市label
  const existingCityLabels = new Set(province.children.map(city => city.label));
  
  // 添加缺失的城市
  for (const cityLabel of provinceData.cities) {
    if (!existingCityLabels.has(cityLabel)) {
      const cityValue = generateCityValue(province.value, cityLabel);
...
// 主函数
function cleanAndCompleteAddressData() {
  try {
    // 读取现有数据
    const currentAddressDataPath = './addressData.ts';
    const currentContent = fs.readFileSync(currentAddressDataPath, 'utf8');
    
    // 提取数组内容
    const dataMatch = currentContent.match(/const.*=\s*\[(.*)\];/s);
    if (!dataMatch) {
      throw new Error('无法提取数据数组');
    }
    
    let provinces = JSON.parse(`[${dataMatch[1]}]`);
    console.log(`读取现有数据，包含 ${provinces.length} 个省级行政区`);
    
    // 创建备份
    const backupFileName = `addressData_backup_${Date.now()}_cleaned.ts`;
    fs.writeFileSync(backupFileName, currentContent, 'utf8');
    console.log(`已备份现有数据到 ${backupFileName}`);
    
    // 处理每个省份
    for (let i = 0; i < provinces.length; i++) {
      const province = provinces[i];
      console.log(`\n处理 ${province.label} (${province.value})`);
      
      // 清理重复数据
      const cleanedProvince = cleanDuplicateCities(province);
      
      // 补全城市数据
      const completedProvince = completeProvinceCities(cleanedProvince);
      
      provinces[i] = completedProvince;
      console.log(`已处理 ${province.label}，现在有 ${completedProvince.children.length} 个城市`);
    }
    
    // 生成新的数据文件内容
    const newContent = `const completeAddressData = ${JSON.stringify(provinces, null, 2)};
...
`;
    ...
    console.log('\n地址数据清理和补全完成！');
    console.log('已清理重复数据并为所有省份补充完整的城市数据。');
